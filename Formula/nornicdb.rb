...
class Nornicdb < Formula
  ...
  def ask_text(prompt, default = nil)
    suffix = default.nil? || default.empty? ? ": " : " [#{default}]: "
    print "#{prompt}#{suffix}"
  ...
  def maybe_download_models(setup)
    ...
    unless ask_yes_no("Download these models now?", true)
      puts "Skipped model downloads. Add the listed files to #{var}/nornicdb/models before starting profiles that require local AI."
      return
    end
